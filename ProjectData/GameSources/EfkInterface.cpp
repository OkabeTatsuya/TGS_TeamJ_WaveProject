...
namespace basecross {
	...
	void EfkPlay::SetScale(const bsm::Vec3& Scale) {
		auto shptr = m_EfkInterface.lock();
		if (shptr && m_Handle != -1) {
			shptr->m_Manager->SetScale(m_Handle, float(Scale.x), float(Scale.y), float(Scale.z));
		}
	}

	void EfkPlay::SetSpeed(const float& Speed) {
		auto shptr = m_EfkInterface.lock();
		if (shptr && m_Handle != -1) {
			shptr->m_Manager->SetSpeed(m_Handle, Speed);
		}
	}

	void EfkPlay::SetPosition(const bsm::Vec3& Position) {
		auto shptr = m_EfkInterface.lock();
		if (shptr && m_Handle != -1) {
			shptr->m_Manager->SetLocation(m_Handle, ::Effekseer::Vector3D(Position.x, Position.y, Position.z));
		}
	}
