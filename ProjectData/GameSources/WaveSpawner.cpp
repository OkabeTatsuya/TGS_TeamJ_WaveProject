...
@brief ÉQÅ[ÉÄÉIÉuÉWÉFÉNÉgê∂ê¨ÉNÉâÉXÇÃé¿ë‘
*/
...
namespace basecross {
	...
	void WaveSpawner::SpawnObject() {
		if (!GameManager::GetInstance().GetIsStopSpawner() && !m_isStopSpawn) {
			...
			//éüÇ…ê∂ê¨Ç∑ÇÈîgÇÃç¿ïWÇéÊÇ¡ÇƒÇ¢ÇÈ
			float spawnTime = m_createPosX[m_spawnCount] - m_offScreen;

			//ç°ÇÃãóó£Ç∆éüÇ…ê∂ê¨Ç∑ÇÈç¿ïW
	...
	void WaveSpawner::VisibleObject(int waveTypeNum) {
		...
		for (int i = 0; i < m_waveObject[waveTypeNum].size(); i++) {
			//ÉvÅ[ÉäÉìÉOÇµÇΩÉIÉuÉWÉFÉNÉgÇ©ÇÁìÆÇ©ÇπÇÈÇ‡ÇÃÇíTÇ∑
			...
			//ìÆÇ©ÇπÇÈÇ‡ÇÃÇ™Ç»Ç©Ç¡ÇΩÇÁçÏê¨Ç∑ÇÈ
	...
	void WaveSpawner::CreateBigWave() {
		...
		if (m_isCreateBigWave && !m_waveObject[WaveType::en_waveL][0]->GetIsMove() && !gameManager.GetIsJumpBigWave()) {
			...
			//MessageBox(nullptr, L"WeveSpawner.cpp 141", L"ÔøΩeÔøΩXÔøΩg", MB_OK);
		}
	}
